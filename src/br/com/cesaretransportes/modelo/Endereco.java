...
public class Endereco implements Serializable {
	...
	public enum StatusEndereco {
		...
		public static StatusEndereco criarPorCodigo(int codigo){
			...
			throw new IllegalArgumentException("Status de endere�o inv�lido para o c�digo " + codigo + ". Valor deve ser 0, 1 ou 2.");		
		}
	}

	public void serialize(DataOutputStream dataOut) throws IOException {
		dataOut.writeInt(idEndereco);
		dataOut.writeInt(empresa.getIdEmpresa());
		dataOut.writeInt(cliente.getIdCliente());
		dataOut.writeInt(orcamento.getIdOrcamento());
		dataOut.writeUTF(cidade);
		dataOut.writeUTF(estado);
		dataOut.writeUTF(localizacao);
		dataOut.writeInt(statusEndereco.ordinal());	
	}	
}