 ...
 * 
 */
public class AndroidVeiculoDeletar extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			write(response, "Veiculo excluido com sucesso!");
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
			write(response, "ERRO codigo CNFE14 " + e.getMessage());
		} catch (SQLException e) {
			e.printStackTrace();
			write(response, "ERRO codigo SQLE14 " + e.getMessage());
		} finally {
			try {
				conexao.close();
			} catch (SQLException e) {
				e.printStackTrace();
				write(response, "ERRO codigo SQLE214 " + e.getMessage());
			}
		}
	}
